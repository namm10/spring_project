...
<DIV id="notice">
	<H2>공지사항</H2>
	<UL>
	<jsp:useBean id="NoticeBoardVO" class="com.marketSpring.board.noticeBo.vo.NoticeBoardVO" />
	<jsp:setProperty property="*" name="NoticeBoardVO"/>
	<%-- <%
	   NoticeBoardDAO dao = NoticeBoardDAO.getTitle();
	
	   dao.title(NoticeBoardDAO);
	%> --%>
	<c:forEach  var="i" begin="1" end="5" step="1">
		<li><a href="${contextPath}/board/noticeBo/viewNoticeBoard.do?noticeBoardNO=${i}">${i}dsadads</a></li>
	</c:forEach>
	<c:forEach var="item" items="${dateMap.newbook }" >
		<a href="${contextPath}/goods/goodsDetail.do?goods_id=${item.title }"></a>
	</c:forEach>
	</ul>
</div>
