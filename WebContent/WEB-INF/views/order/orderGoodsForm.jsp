...
<!-- 최종 적립 예정 금액 -->
<c:set var="total_order_point" value="0" />
...
<head>
	...
	function execDaumPostcode() {
		...
		// 상품 주문 페이지가 표시되면 주문자의 휴대폰 번호와 유선 전화번호를 셀렉트 박스에 표시함
		...
		function recently_delivery() {
			...
			$.ajax({
				url : "${contextPath}/order/findRecentlyDelivery.do",
				type : "GET",
				async : false, //false인 경우 동기식으로 처리한다.
				data : { /* 주문 자 id 값을 컨트롤러로 전송 */
					member_id : '${orderer.member_id}'
				},
				success : function(data) {
					if (data.Code == 0) {
						recentlyOrderer = data.data.member_id;
	
						e_receiver_name.value = data.data.receiver_name;
						e_hp1.value = data.data.receiver_hp1;
						e_hp2.value = data.data.receiver_hp2;
						e_hp3.value = data.data.receiver_hp3;
	
						e_tel1.value = data.data.receiver_tel1;
						e_tel2.value = data.data.receiver_tel2;
						e_tel3.value = data.data.receiver_tel3;
	
						e_zipcode.value = data.data.delivery_zipcode;
						e_roadAddress.value = data.data.delivery_roadAddress;
						e_jibunAddress.value = data.data.delivery_jibunAddress;
						e_namujiAddress.value = data.data.delivery_namujiAddress;
	
						h_zipcode.value = data.data.delivery_zipcode;
						h_roadAddress.value = data.data.delivery_roadAddress;
						h_jibunAddress.value = data.data.delivery_jibunAddress;
						h_namujiAddress.value = data.data.delivery_namujiAddress;
		...
		function fn_usePoint() {
			var e_card = document.getElementById("discount_juklip");
			var salesPrice = document.getElementById("p_totalSalesPrice");
			
			if (window.event.keyCode == 13) { // 할인 금액 입력 후 엔터키 입력 시 적용
				if(e_card.value > ${orderer.member_point } ){
					alert("보유 포인트 이상 사용은 불가능 합니다.");
					e_card.value = 0;
				}else{
		            fn_pointApply(); // 입력한 적립금 적용
				}
			}
			
			$('#discount_juklip').focusout(function() { // input 창밖으로 focus가 나갈때  event
				if(e_card.value > ${orderer.member_point } ){
					alert("보유 포인트 이상 사용은 불가능 합니다.");
					e_card.value = 0;
				}else{
		...
		// 할인정보 - 적립금 모두 사용하기
		function fn_usePointAll() {
			var e_card = document.getElementById("discount_juklip");
			var p_salesPrice = document.getElementById("p_totalSalesPrice");
			var h_salesPrice = document.getElementById("h_total_sales_price");
			
			if($("#pointUseChk").is(":checked")){
				e_card.value = ${orderer.member_point };
	        }else{
				e_card.value = 0;
	        }
		...
		// 입력한 적립금 적용
		function fn_pointApply() {
			var e_card = document.getElementById("discount_juklip");	// 입력한 적립금액
			var p_salesPrice = document.getElementById("p_totalSalesPrice");	// 총 할인 금액
			var h_salesPrice = document.getElementById("h_total_sales_price");	// 총 할인 금액
			
			p_salesPrice.innerHTML = parseInt(${total_discount_price } + e_card.value) + "원";
			h_salesPrice.value = parseInt(${total_discount_price } + e_card.value);

			// 최종 결제 금액 계산
			var totalPrice = document.getElementById("h_totalPrice").value;	// 할인 전 총 상품금액
			var p_finalTotalPrice = document.getElementById("p_final_totalPrice");
			var h_finalTotalPrice = document.getElementById("h_final_total_Price");
			
			p_finalTotalPrice.innerHTML = "<font size='15'>"+parseInt(totalPrice- h_salesPrice.value)+"원 </font>";
			h_finalTotalPrice.value = parseInt(totalPrice - h_salesPrice.value);

			// 예정 적립금 계산 5%적립
			var p_expected_point = document.getElementById("p_expected_point");
			var h_expected_point = document.getElementById("h_expected_point");
			p_expected_point.innerHTML = "<font size='3px'><b>"+parseInt(h_finalTotalPrice.value*0.05)+" 원 적립 예정</b></font>";
			h_expected_point.value = parseInt(h_finalTotalPrice.value*0.05);
		}
		...
		function fn_show_order_detail() {
			...
			order_point = parseInt(h_expected_point.value); // 예상 적립금
			...
			var i_order_point = document.getElementById("h_expected_point");
			...
			jibunAddress = i_jibunAddress.value;
			namujiAddress = i_namujiAddress.value;

			order_point = i_order_point.value;
			...
			var p_order_point = document.getElementById("p_order_point");
			...
			p_order_point.innerHTML = order_point + "원";
			imagePopup('open');
		}

		// [최종 결제하기] 클릭하면 <input> 태그를 동적으로 생성한 후 주문창에서 입력한 수령자 정보를 설정하여 컨트롤러로 전송
		function fn_process_pay_order() {
			...
			var i_delivery_roadAddress = document.createElement("input");
			var i_delivery_jibunAddress = document.createElement("input");
			var i_delivery_namujiAddress = document.createElement("input");
			
			var i_order_point = document.createElement("input");
			...
			i_delivery_zipcode.name = "zipcode";
			i_delivery_roadAddress.name = "roadAddress";
			i_delivery_jibunAddress.name = "jibunAddress";
			i_delivery_namujiAddress.name = "namujiAddress";

			i_order_point.name = "order_point";
			...
			i_delivery_zipcode.value = zipcode;
			i_delivery_roadAddress.value = roadAddress;
			i_delivery_jibunAddress.value = jibunAddress;
			i_delivery_namujiAddress.value = namujiAddress;

			i_order_point.value = order_point;
			...
			formObj.appendChild(i_delivery_roadAddress);
			formObj.appendChild(i_delivery_jibunAddress);
			formObj.appendChild(i_delivery_namujiAddress);

			formObj.appendChild(i_order_point);
			...
			<tbody align=center>
				...
				<tr>
					<c:forEach var="item" items="${myOrderList }">
						...
						<td>
							<h2>${item.order_goods_size}cm</h2>
						...
						<td><h2>0원</h2></td>
						<td>
							<h2><fmt:parseNumber var="final_expected_point" value="${(item.goods_sales_price * item.order_goods_qty) * 0.05 }" integerOnly="true" />${final_expected_point }원</h2>
						</td>
						<td>
				...
				<c:set var="total_order_point" value="${total_order_point + (item.goods_sales_price * item.order_goods_qty) * 0.05 }" />
				</c:forEach>
			</tbody>
		</table>
		<div class="clear"></div>
			...
			<table>
				<tbody>
					<tr class="dot_line">
						...
						<td>
							<input type="radio" id="delivery_method" name="delivery_method" value="택배배송" checked>택배배송&nbsp;&nbsp;&nbsp; 
							<input type="radio" id="delivery_method" name="delivery_method" value="신선배송">신선배송&nbsp;&nbsp;&nbsp; 
						</td>
					</tr>
					<tr class="dot_line">
						...
						<td>
							...
							<input type="radio" name="delivery_place" onClick="recently_delivery()" value="최근배송지">최근배송지&nbsp;&nbsp;&nbsp;
						</td>
					</tr>
					<tr class="dot_line">
					...
					<tr class="dot_line">
						...
						<td>
							...
							</select> - <input size="10px" type="text" id="hp2" name="hp2" value="${orderer.hp2 }"> - <input size="10px" type="text" id="hp3" name="hp3" value="${orderer.hp3 }"><br> <br>
					...
					<tr class="dot_line">
						...
						<td>
							...
							<p>
								지번 주소:<br>
								<input type="text" id="roadAddress" name="roadAddress" size="50" value="${orderer.roadAddress }" /><br><br> 
								도로명 주소: <input type="text" id="jibunAddress" name="jibunAddress" size="50" value="${orderer.jibunAddress }" /><br><br>
								나머지 주소: <input type="text" id="namujiAddress" name="namujiAddress" size="50" value="${orderer.namujiAddress }" />
							</p>
		...
		<div>
			...
			<table>
				<tbody>
					<tr class="dot_line">
						<td><h2>이름</h2></td>
						<td><input type="text" value="${orderer.member_name}" size="15" /></td>
					</tr>
					<tr class="dot_line">
						<td><h2>핸드폰</h2></td>
						<td><input type="text" value="${orderer.hp1}-${orderer.hp2}-${orderer.hp3}" size="15" /></td>
					</tr>
					<tr class="dot_line">
						<td><h2>이메일</h2></td>
						<td><input type="text" value="${orderer.email1}@${orderer.email2}" size="15" /></td>
					</tr>
				</tbody>
			</table>
		</div>
			...
			<table>
				<tbody>
					<tr class="dot_line">
						<td width=100>적립금</td>
						<td>
							<!-- <input name="discount_juklip" id="discount_juklip" type="text" size="10" onkeyup="fn_usePoint()" />원 / 1000원 &nbsp;&nbsp;&nbsp; --> 
							<input name="discount_juklip" id="discount_juklip" type="text" size="10" onkeyup="fn_usePoint()" /> 원 / ${orderer.member_point }   
							<input type="checkbox" id="pointUseChk" onClick="fn_usePointAll()" /> 모두 사용하기
						</td>
					</tr>
				</tbody>
			</table>
			...
			<tbody>
				...
				<tr>
					<td colspan="8" style="padding-left:25px">
						<p id="p_expected_point">
							<font size="3px">
								<%-- <b>${final_expected_point } 원 적립 예정</b> --%>
								<b>
									<fmt:parseNumber var="final_total_order_point" value="${total_order_point }" integerOnly="true" />
									${final_total_order_point } 원 적립 예정
								</b>
							</font>
						</p>
						<input id="h_expected_point" type="hidden" value="${final_total_order_point}" />
						<%-- <input id="order_poin" type="hidden" value="${final_expected_point}" /> --%>
					</td>
				</tr>
			</tbody>
		</table>
			...
			<div class="detail_table">
				...
				<table>
					<tbody align=left>
						...
						<tr>
							<td width=200px>예정적립금액:</td>
							<td>
							<p id="p_order_point">예정적립금액</p>
							</td>
						</tr>
						<tr>
